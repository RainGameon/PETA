...
class EpisodeBatch:
    ...
    def update(self, data, bs=slice(None), ts=slice(None), mark_filled=True):
        ...
        for k, v in data.items():
            ...
            # print(f'SMY —————————————— k = {k}; v.shape={v.shape}; target.shape={target[k].shape}')
...
class Prioritized_ReplayBuffer(EpisodeBatch):
    ...
    def sample(self, batch_size, newest=False):
        ...
        if newest and self.episodes_in_buffer >= batch_size:
            ...
            # print(f'batch.shape = {batch.shape}, ep_ids.shape = {(np.arange(batch_size)).shape}')
            ...
            assert (ep_ids != None)
            # ep_ids = np.random.choice(self.episodes_in_buffer, batch_size, replace=False)
            # batch = self[ep_ids]['state']
            # print(f'batch.shape = {batch.shape}, ep_ids.shape = {(ep_ids).shape}')
