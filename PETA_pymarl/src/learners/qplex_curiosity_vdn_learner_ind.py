...
'''使用transformer进行奖励分解得到个人奖励，然后分别加给各自Qi'''
class QPLEX_curiosity_vdn_Learner_ind:
    def __init__(self, mac, scheme, logger, args, groups=None):
        ...
        if self.args.rewardnet_used:
            # self.rewardnet = ModifiedSelfAttention(args.obs_shape + args.n_actions, args.rewardnet_head,
            #                                        args.rewardnet_embed_size)
            self.rewardnet = rewardRNNet(args.obs_shape + args.state_shape + 1,args)
            self.rewardnet_params = list(self.rewardnet.parameters())
            self.rewardnet_optimiser = Adam(params=self.rewardnet_params, lr=args.predictor_lr)
    ...
    def sub_train(self, batch: EpisodeBatch, t_env: int, episode_num: int, mac, mixer, optimiser, params,intrinsic_rewards,
        ...
        '''————————————————————计算individual reward——————————————————'''
        if self.args.rewardnet_used:
            self.rewardnet.init_hidden(batch.batch_size)
            individual_R_e = self.rewardnet.forward(batch, batch.max_seq_length)[:,:-1].squeeze(3) # [batch, time, agent]
            # print(f'individual_R_e.shape={individual_R_e.shape}; mask.shape = {mask.shape}')
            individual_R_e_detach = individual_R_e.clone().detach() * mask * self.args.rewardnet_weight  # 0.1 #(b,t,n)
        ...
        if mixer is not None:
            if self.args.mixer == "dmaq_qatten":
                if self.args.individual_curiostiy and self.args.rewardnet_used:
                    ans_chosen, q_attend_regs, head_entropies = \
                        mixer(chosen_action_qvals - individual_R_e_detach * 0.05 - intrinsic_rewards * 0.1, batch["state"][:, :-1], is_v=True)
                    ans_adv, _, _ = mixer(chosen_action_qvals - individual_R_e_detach  * 0.05- intrinsic_rewards * 0.1, batch["state"][:, :-1],
        ...
        if self.args.optimality_incentive:
            ...
            if self.args.individual_curiostiy:
                # intrinsic_rewards_total = intrinsic_rewards.mean(dim=-1, keepdim=True).detach()
        ...
        '''———————————————更新reward net参数———————————'''
        if self.args.rewardnet_used:
            # 预测的个人奖励individual_R_e(b,t,n_agent,1)先与表示生死对mask相乘，然后求和得到total_reward
            alive = batch["alive"][:,:-1]  # (b,t,n_Agent)
            # print(f' alive.shape = {alive.shape}')

            total_reward = (individual_R_e * alive * mask).mean(dim=2, keepdim=True)
            # print(f'individual_R_e.shape={individual_R_e.shape}; alive.shape = {alive.shape}; total_reward.shape={total_reward.shape}')

            rewardnet_loss = F.mse_loss(total_reward, rewards * 10, reduction="mean")
            self.rewardnet_optimiser.zero_grad()
            rewardnet_loss.backward()
            rewardnet_grad_norm = th.nn.utils.clip_grad_norm_(self.rewardnet_params, self.args.grad_norm_clip)
            self.rewardnet_optimiser.step()
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval: # learner_log_interval 10000
            ...
            self.logger.log_stat("target_mean", (targets * mask).sum().item() / (mask_elems * self.args.n_agents),
                                 t_env)
            if self.args.rewardnet_used:
                self.logger.log_stat("pred_total_reward", (total_reward * mask).sum().item() / mask_elems, t_env)
                self.logger.log_stat("rewardnet_loss", rewardnet_loss, t_env)
    ...
    def cuda(self):
        ...
        if self.args.rewardnet_used:
            to_cuda(self.rewardnet, self.args.device)
        if self.mixer is not None:
