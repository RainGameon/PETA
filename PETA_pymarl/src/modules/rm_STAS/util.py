...
def make_env(scenario_name, discrete_action_input=True, benchmark=False):
    ...
    env = MultiAgentEnv(world, scenario.reset_world, scenario.reward, scenario.observation,
                        scenario.info if scenario_name.startswith('simple_tag') else None,
...
def make_train_step_temporal(model, loss_fn, optimizer, device, reg=False, alpha=None, beta=None):
    # Builds function that performs a step in the train loop
    if reg: assert alpha is not None

    def train_step(states, actions,mask, episode_return, episode_length):
        # Sets model to TRAIN mode
        model.train()

        # Makes predictions: pred_return: [batch_size, time length]
        pred_rewards = model(states, actions,mask, episode_length).squeeze()
        

        # Computes loss
        # main_mask = (torch.arange(pred_rewards.shape[-1])[None, :].to(device) <= (episode_length - 1).reshape(-1, 1).squeeze()[   :, None]).to(device).squeeze()
        # main_mask = main_mask.unsqueeze(1).repeat(1, 1, 1)  # No need for n_agent dimension anymore
        main_mask  = mask.squeeze()
        # print(f'SMY: pred_rewards={pred_rewards.shape}; main_mask={main_mask.shape}; episode_return={episode_return.shape}')
        # Loss now sums across all time steps (dim=[1, 2] removed)
        # print(f'——————————————————————start————————————————————')
        # print('episode_return:', episode_return.min(), episode_return.max())
        # print('pred_rewards', pred_rewards.min(), pred_rewards.max())
        # print(main_mask.min(), main_mask.max())
        
        pre_return = (pred_rewards * main_mask).sum(dim=[1]) 
        # print('pre_return:',pre_return.min(), pre_return.max())
        loss_mse = loss_fn(episode_return ,pre_return)  # No need for dim=[1,2], sum only on time dimension
        if reg:
            pred_rewards_mean = torch.sum(pred_rewards * main_mask, dim=-1) / torch.sum(main_mask, dim=-1)  # Mean over the time axis
            pred_rewards_mean = pred_rewards_mean.unsqueeze(-1).repeat(1, pred_rewards.shape[-1])  # Broadcasting mean
            ...
            # print('loss:',loss_mse, reg_loss)
            # print(f'——————————————————————end————————————————————')
            loss = loss_mse + reg_loss   #alpha * reg_loss
            
        loss.backward()
        optimizer.step()
        optimizer.zero_grad()
        return loss.item(), pred_rewards
...
def make_train_step(model, loss_fn, optimizer, n_agent, device, reg=False, alpha=None):
    ...
    def train_step(states, actions, episode_return, episode_length):
        ...
        main_mask = (torch.arange(pred_rewards.shape[-1])[None, :].to(device) <= (episode_length - 1).reshape(-1,
                                                                                                              1).squeeze()[
                                                                                 :, None]).to(device).squeeze()
